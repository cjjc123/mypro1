...
#define _DIR_PREFIX_FILE_   "html/dir_header.html"  // 头部
#define _DIR_TAIL_FILE_     "html/dir_tail.html"    // 尾部
...
int send_dir(struct bufferevent *bev,const char *strPath)
{
    ...
    while( (dent= readdir(dir) ) ) //遍历 读取 目录
    {
        struct stat sb;
        stat(dent->d_name,&sb);
...
{
    strdecode(path, path);//将中文问题 转码成utf-8格式 的字符串
...
void read_cb(struct bufferevent *bev, void *ctx)
{
    ...
    char method[10];   // 方法 （请求 类型）
    char path[256];    // 路径 （文件 类型）
    char protocol[10]; // 协议
    ...
    if(ret > 0)
    {
        ...
        if(strcasecmp(method,"get") == 0) //比较 字符串 （相同 返回0）
        {
            //处理客户端的请求
            char bufline[256];
...
{
    //定义与客户端通信的 bufferevent（事件）
    struct event_base *base = (struct event_base *)arg; //地基
    struct bufferevent *bev = bufferevent_socket_new(base, fd, BEV_OPT_CLOSE_ON_FREE);

    bufferevent_setcb ( bev, read_cb, NULL, bevent_cb, base);//设置回调

    bufferevent_enable(bev,EV_READ|EV_WRITE);//启用读和写 （设置bufferevent 读、写事件 生效）
}
...
// 改变 当前进程 的工作目录
	...
	chdir(workdir);         // getenv( )函数 ：获取 环境变量 的值
    ...
    struct evconnlistener * listener =evconnlistener_new_bind(base, listen_cb, base,
                                                         ...
                                                         -1 , (struct sockaddr *)&serv,  sizeof(serv) );//连接监听器

    event_base_dispatch(base);  // 进入 循环
